...
class DiscordBotApi:
    ...
    @classmethod
    def sync_server_members(cls, server_id, bot_token):
        """
        Синхронизирует список участников сервера
        """
        try:
        ...
        members_data = cls.get_server_members(server_id, bot_token)
        if not members_data:
            logger.error(f"Failed to get members for server {server.server_name}")
            return False

        discord_users = CustomUser.objects.filter(discord_user__isnull=False)
        discord_id_map = {user.discord_user.discord_id: user for user in discord_users}
        ...
        for member_data in members_data:
            ...
            if not discord_id or discord_id not in discord_id_map:
                continue

            user = discord_id_map[discord_id]
            ServerMember.objects.get_or_create(user=user, server=server)

        server.member_count = len(members_data)
        server.save()

        logger.info(f"Synchronized members for server {server.server_name}")
        return True

    @staticmethod
    def get_server_members(server_id, bot_token):
        """
        Получает список участников сервера через Discord API

        Args:
            server_id (str): ID сервера Discord
            bot_token (str): Токен бота Discord

        Returns:
            list: Список участников сервера или None в случае ошибки
        """
        members_url = f"https://discord.com/api/guilds/{server_id}/members?limit=1000"
        headers = {"Authorization": f"Bot {bot_token}"}

        try:
            response = requests.get(members_url, headers=headers)
            if response.status_code != 200:
                logger.error(f"Failed to get server members: {response.status_code}, {response.text}")
                return None

            return response.json()
        except requests.RequestException as e:
            logger.error(f"Request error during server members fetch: {str(e)}")
            return None
        except Exception as e:
            logger.error(f"Error while processing server members: {str(e)}")
            return None